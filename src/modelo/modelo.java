...
//obtener el margen de ganancia de un producto
    
    public double ganancia (int producto, int cantidad){
        ...
        ResultSet rs = null;
        try {
            String sql = "SELECT costo,publico FROM inventario WHERE idinventario=?";
            ps = cn.prepareStatement(sql);
            ps.setInt(1, producto);
            rs = ps.executeQuery();
            while (rs.next()) {
                costo=rs.getInt("costo");
                publico=rs.getInt("publico");
            }            
        ...
        costo=costo/100;
        publico=publico/100;
        Double cant =(double) cantidad/100;
        ...
        int ganancia=publico-costo;
        System.out.println("la ganancia es "+ganancia);
        System.out.println("");
        System.out.println("Cantidad es "+cant);        
        Double meGano=(double)ganancia*cant;
        System.out.println("me gano "+meGano);
        System.out.println("");
        return meGano;
...
//Imprimir = 1ra linea de la columa de 1 a 32
        ...
        try {
            ...
            "ticket.cantidad as cantidad, ticket.total as total, ticket.porcentaje as descuento from ticket " +
            ...
            String strDescuento=rs.getString("descuento");
            
            Double vprecio=Double.valueOf(precio);
            Double vCantidad=Double.valueOf(cantidad);
            total=Double.valueOf(strTotal);
            descuento=Double.valueOf(strDescuento);
            
            Double Dsubtotal=Math.ceil(vCantidad*vprecio);
            subtotal +=Dsubtotal;
            ...
            String inp= printer.alinharADireita(7,Dsubtotal.toString());
        ...
        printer.printTextWrap(linea, 1, 0, 32, "--------------------------------");
        linea++;
        printer.printTextWrap(linea, 1, 0, 32, "SubTotal                " + subtotal);
        linea++;
        printer.printTextWrap(linea, 1, 0, 32, "Descuento               " + descuento);
        linea++;
        printer.printTextWrap(linea, 1, 0, 32, "Total de compra         " + total);
        linea++;
        if (recibo== 0.0){
            printer.printTextWrap(linea, 1, 0, 32, "Ticket Reimpreso");
            linea++;
            printer.printTextWrap(linea, 1, 0, 32, "Papel sin valor");
        }else{
        printer.printTextWrap(linea, 1, 0, 32, "Dinero recibido         " + recibo);
        linea++;
        printer.printTextWrap(linea, 1, 0, 32, "Dinero devuelto         " + (recibo-total));
        }
        linea++;
        printer.printTextWrap(linea, 1, 0, 32, "================================");
        linea++;
